...
class EventService implements EventRepositoryInterface
{
    ...
    public function getCheckPayment($users_id, $events_id)
    {
        return DB::table('payment')
            ->join('events_tickets', 'events_tickets.id', 'payment.package_id')
            ...
            ->select('payment.qr_code', 'events_tickets.type as package', 'events_tickets.title as ticket_title', 'payment.status', 'payment.code_payment')->first();
    }
    ...
    {
        $find = Payment::where('code_payment', $qrcode)->where('events_id', '14')->first();
        if ($find) {
